...
pub fn test_conversion(color: Rgb<f32>) {
    ...
    input_buffer_slice
        ...
        .clone_from_slice(bytemuck::cast_slice(&local_buffer));

    in_buffer.unmap();

    let mut encoder =
        device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
    encoder.copy_buffer_to_buffer(&in_buffer, 0, &storage_buffer, 0, 4 * 3);
    {
    ...
    encoder.copy_buffer_to_buffer(&storage_buffer, 0, &out_buffer, 0, 4 * 3);
    ...
    device.poll(wgpu::Maintain::Wait);
    local_buffer.copy_from_slice(&bytemuck::cast_slice(
        &*output_buffer_slice.get_mapped_range(),
    ));
