...
use palette::{Hsl, Srgb, FromColor, IntoColor, Saturate};
...
pub fn old_adjust_exposure(buffer: &Vec<Vec<f64>>, added_value: f32) -> Vec<Vec<f64>> {
...
pub fn old_convert_to_grayscale(buffer: &Vec<Vec<f64>>) -> Vec<f64> {
...
pub fn old_combine_grayscale_with_colored(
...
pub fn old_adjust_contrast(buffer: &Vec<Vec<f64>>, added_value: f32) -> Vec<Vec<f64>> {
    let added_value: f64 = norm_range(-0.5..=0.5, added_value as f64) + 1.0;

    let mut gray_buffer: Vec<f64> = old_convert_to_grayscale(&buffer);
    ...
    return old_combine_grayscale_with_colored(&gray_buffer, &buffer);
}
