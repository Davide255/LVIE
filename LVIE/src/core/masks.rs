...
impl Mask {
    ...
    pub fn generate_line_for_slint(
        ...
        let bcp: Vec<[[f32; 2]; 2]> = (&self.bezier_control_points)
            .into_iter()
            .map(|k| {
                let m: Vec<[f32; 2]> = k
                    .into_iter()
    ...
    pub fn generate_control_point_connection_lines_for_slint(
        ...
        for i in 0..self.mask_points.len() - 1 {
            ...
            line.append(&mut generate_linespace(
                p[i][1][0],
                p[i][1][1],
                self.mask_points[i + 1][0],
                self.mask_points[i + 1][1],
                100,
            ));
        }
        ...
        if self.closed {
            line.append(&mut generate_linespace(
                self.mask_points.last().unwrap()[0],
                self.mask_points.last().unwrap()[1],
                p.last().unwrap()[0][0],
                p.last().unwrap()[0][1],
                100,
            ));
            line.append(&mut generate_linespace(
                p.last().unwrap()[1][0],
                p.last().unwrap()[1][1],
                self.mask_points[0][0],
                self.mask_points[0][1],
                100,
            ));
        }
    ...
    fn generate_line(&self, width: f32, height: f32) -> Vec<[f32; 2]> {
    ...
    fn generate_track<P>(
        ...
        width: u32,
        height: u32,
    ) -> Result<image::ImageBuffer<P, Vec<P::Subpixel>>, MaskError>
    where
        P: image::Pixel + std::fmt::Debug + LVIElib::traits::ToHsl + 'static,
        P::Subpixel:
            image::Primitive + std::fmt::Debug + Send + Sync + Default + LVIElib::traits::Scale,
    {
        if !self.closed {
            ...
            for k in self.generate_line(width as f32, height as f32) {
                ...
                for i in -1..=1 {
                    for j in -1..=1 {
                        points[width as usize * (y + j) as usize + (x + i) as usize] =
                            255u8.scale::<P::Subpixel>();
                    }
                }
            }
    ...
    where
        ...
        P::Subpixel: image::Primitive
            ...
            + Sync
            + Default
            + LVIElib::traits::Scale
            + Send
            + Sync,
    {
        let track: image::ImageBuffer<P, Vec<P::Subpixel>> =
            self.generate_track(image.width(), image.height())?;
            ...
            for (j, z) in k.into_iter().enumerate() {
                if *z == [-1.0, -1.0] {
                    step[j] = {
                        ...
                        [
                            l[0] + ((1.0 + 1.0 * j as f32) * dx / 3.0),
                            l[1] + ((1.0 + 1.0 * j as f32) * dy / 3.0),
                        ]
                    };
                } else {
                    step[j] = *z;
    ...
    pub fn update_point(&mut self, index: usize, point: [f32; 2]) -> Result<(f32, f32), MaskError> {
        if index >= self.mask_points.len() {
            ...
            if index < self.bezier_control_points.len()
                && self.bezier_control_points[index][0] != [-1.0, -1.0]
            {
...
fn generate_linespace(
    from_x: f32,
    from_y: f32,
    to_x: f32,
    to_y: f32,
    steps: usize,
) -> Vec<slint::ModelRc<f32>> {
