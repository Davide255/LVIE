...
use image::{RgbImage, Pixel, RgbaImage, Primitive};
    ...
    P::Subpixel: Scale + Primitive + std::fmt::Debug + Send + Sync
{
    ...
    let out = Arc::new(Mutex::new(vec![P::Subpixel::DEFAULT_MIN_VALUE; (width*height*4) as usize]));
    ...
    let mut alphas: Vec<P::Subpixel> = vec![P::Subpixel::DEFAULT_MIN_VALUE; (width*height) as usize];

    for row in 0..height{
        for col in 1..width{
            let pixel = img.get_pixel(col, row).channels();
            sum_r[idx(&row, &col, &width, 1) as usize] = <f64 as NumCast>::from(pixel[0]).unwrap() + sum_r[idx(&row, &(col - 1), &width, 1) as usize];
            sum_g[idx(&row, &col, &width, 1) as usize] = <f64 as NumCast>::from(pixel[1]).unwrap() + sum_g[idx(&row, &(col - 1), &width, 1) as usize];
            sum_b[idx(&row, &col, &width, 1) as usize] = <f64 as NumCast>::from(pixel[2]).unwrap() + sum_b[idx(&row, &(col - 1), &width, 1) as usize];
            alphas[idx(&row, &col, &width, 1) as usize] = pixel[3];
        }
    }
        ...
        for col in 0..width {
            ...
            for color in 0..4 {
                if color == 3 { r.push(alphas[idx(&row, &col, &width, 1) as usize]); }
                    ...
                    // (q, n). The sum of all the pixels in the box surrounding the
                    // pixel is then equal to `d - (c + b - a)`.
                    let a = {if y_min < 1 || x_min < 1
                        { 0.0 } else { 
                            sums_color[idx(&(y_min - 1), &(x_min - 1), &width, 1) as usize]
                        }
                    };

                    let b = {if y_min < 1 { 0.0 }
                        else { sums_color[idx(&(y_min - 1), &x_max, &width, 1) as usize] }};

                    let c = {if x_min < 1 { 0.0 }
                        else { sums_color[idx(&y_max, &(x_min - 1), &width, 1) as usize]}};
                    let d = sums_color[idx(&y_max, &x_max, &width, 1) as usize];
