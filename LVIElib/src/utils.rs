use crate::{
    ...
    oklab::{Oklab, OklabImage, OklabaImage},
    traits::{Scale, ToHsl, ToOklab},
};
use half::f16;
...
pub fn convert_rgb_to_rgba(img: &image::RgbImage) -> image::RgbaImage {
    ...
    img.enumerate_pixels()
        .par_bridge()
        .for_each(move |(x, y, pixel)| {
            out_w.lock().unwrap().put_pixel(
                x,
                y,
                image::Rgba([pixel.0[0], pixel.0[1], pixel.0[2], 255]),
            );
        });
    ...
    img.enumerate_pixels()
        ...
        .for_each(move |(x, y, pixel)| {
            out_w
                .lock()
                .unwrap()
                .put_pixel(x, y, image::Rgb([pixel.0[0], pixel.0[1], pixel.0[2]]));
        });
...
pub fn f32_vec_to_f16_vec(src: &Vec<f32>, size: (u32, u32)) -> Vec<f16> {
    ...
    (0..size.1).into_par_iter().for_each(|x| {
        ...
        vt.lock().unwrap()[(x * size.0) as usize..(x * size.0 + size.0) as usize]
            .copy_from_slice(&r);
    });
    ...
    P::Subpixel: Scale + Send + Sync + Primitive + std::fmt::Debug + std::fmt::Debug,
{
    ...
    ImageBuffer::<P, Vec<P::Subpixel>>::from_vec(
        img.width(),
        img.height(),
        Arc::try_unwrap(out).unwrap().into_inner().unwrap(),
    )
...
pub unsafe fn convert_oklaba_to_rgba<P>(
    img: &OklabaImage,
