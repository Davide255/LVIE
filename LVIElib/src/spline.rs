...
fn solve_tridiagonal_system(up: Vec<f32>, mid: Vec<f32>, down: Vec<f32>, b: Vec<f32>) -> Vec<f32> {
    ...
    delta[n - 1] =
        (b[n - 1] - down[n - 2] * delta[n - 2]) / (mid[n - 1] - gamma[n - 2] * down[n - 2]);
...
pub fn spline_coefficients(
    data: &Vec<f32>,
    xs: &Vec<f32>,
    cstr: SplineConstrains,
) -> Vec<[f32; 4]> {
    let n = data.len();
    ...
    match cstr {
        ...
        SplineConstrains::SecondDerivatives(start, end) => {
            ...
            b[n - 1] = 6.0 * (data[n - 1] - data[n - 2]) / (xs[n - 1] - xs[n - 2])
                + end * (xs[n - 1] - xs[n - 2]);
        }
    }
...
pub fn apply_1st_derivative(val: f32, spline: &Vec<[f32; 4]>, x: &Vec<f32>) -> f32 {
    for i in 0..x.len() - 1 {
        if x[i] <= val && val < x[i + 1] {
            let z = (val - x[i]) / (x[i + 1] - x[i]);
            return (spline[i][1] + 2.0 * spline[i][2] * z + 3.0 * spline[i][3] * z * z)
                / (x[i + 1] - x[i]);
        }
    }
...
pub fn create_plot_view<S: plotters::style::SizeDesc + num_traits::NumCast>(
    ...
    xs: &Vec<f32>,
    ys: &Vec<f32>,
    x_max: Option<std::ops::Range<f32>>,
    y_max: Option<std::ops::Range<f32>>,
    margins: (S, S, S, S),
    ...
    chart.draw_series(LineSeries::new(
        (0..=5000).map(|x| x as f32 / 50.0).map(|x| {
            (x / 100.0, {
                apply_curve(
                    x,
                    {
...
mod tests {
    ...
    fn tridiagonal_solve_test() {
        ...
        let data: Vec<f32> = vec![
            0.0, 12.3, 32.3, 134.4, 13.4, 52.7, 43.6, 38.9, 27.0, 15.3, 22.0, 13.0, 23.3, 44.4,
            64.6, 178.4, 2.2, 5.5233, 55.76, 3.534, 123.321, 123.42, 72.0,
        ]
        .iter()
        .map(|x| x / 178.4)
        .collect();
    ...
    fn harder_tridiagonal_test() {
        ...
        let up = vec![
            1.0, 23.0, 42.5, 123.9, 53.3, 0.005, 23.555, 34.04, 49.24, 294.94, 24.68, 0.5959,
            34.68, 04.85,
        ];
        let mid = vec![
            4.0, 178.4, 2.2, 5.5233, 55.76, 3.534, 123.321, 123.42, 72.0, 29.58, 07.38, 348.85,
            0.004, 12.45, 49.34,
        ];
        let down = vec![
            1.0, 28.58, 23.52, 49.96, 48.92, 237.482, 12.23, 0.594, 0.0043, 23.53, 58.853, 29.95,
            57.37, 95.85,
        ];

        // Solve via matrix inversion
        let A = DMatrix::from_fn(n, n, |r, c| match r as isize - c as isize {
            -1 => up[r],
            0 => mid[r],
            1 => down[c],
            _ => 0.0,
